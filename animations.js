(() => {
  ...
  const icons = {
    ...
    python: '<svg class="skill-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3c-3 0-4 1.5-4 4v2h7v2H8c-3 0-5 1.5-5 4s2 4 5 4h2v-3c0-2 1-3 3-3h4c3 0 4-2 4-5V7c0-2.5-2-4-5-4h-4Z"></path><circle cx="14.5" cy="6.5" r=".7" fill="currentColor" stroke="none"></circle><path d="M12 21c3 0 4-1.5 4-4v-2H9v-2h7c3 0 5-1.5 5-4s-2-4-5-4h-2v3c0 2-1 3-3 3H7c-3 0-4 2-4 5v1c0 2.5 2 4 5 4h4Z"></path><circle cx="9.5" cy="17.5" r=".7" fill="currentColor" stroke="none"></circle></svg>'
  };
  ...
  function setupIcons() {
    document.querySelectorAll(".skill").forEach((skill) => {
      ...
      if (text === "python") 
        addIcon(skill, icons.python);
      else if (text === "sql") 
        addIcon(skill, icons.database);
      else if (text.includes("data analytics") || text.includes("power bi")) 
        addIcon(skill, icons.chart);
      else if (text.includes("github")) 
        addIcon(skill, icons.github.replace('class="link-icon"', 'class="skill-icon"'));
      else 
        addIcon(skill, icons.code);
    });
